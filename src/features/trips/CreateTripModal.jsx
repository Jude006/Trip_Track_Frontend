...
const CreateTripModal = ({ trip, onClose, onTripCreated, onTripUpdated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(error.response?.data?.error || 'Failed to save trip. Please try again.');
        ...
        <form onSubmit={handleSubmit} className="space-y-4 dark:text-dark-text-secondary">
          ...
          <div className="flex justify-end space-x-3 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 border rounded-lg dark:border-gray-600 text-surface dark:text-dark-text-primary"
              disabled={uploading}
            >
              Cancel
            </button>
